let writeQuestions = [
 ["Held hands romantically?", .01],
 ["Been on a date?", .01],
 ["Been in a relationship?", .01],
...
for (let i = 0; i < writeQuestions.length; i++) {
  totalweight += writeQuestions[i][1]
}
...
;
function Question(question, weight) {
...
for (let i = 0; i < writeQuestions.length; i++) {
  questions.push(new Question(writeQuestions[i][0], writeQuestions[i][1]));
}
...
function submit() {
  ...
  for (let i = 0; i < questions.length; i++) {
    cb = document.getElementById(i);
    if (cb.checked) {
      finalScore -= (questions[i].weight/totalweight);
    }
  }
